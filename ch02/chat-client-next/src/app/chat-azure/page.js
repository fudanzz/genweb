...
const Chat = () => {
  ...
  const { messages, isLoading, handleSubmit, inputValue, setInputValue } = useChatFormSubmit((text) => getAssistantResponse(text, '/chat-azure/api'));
    ...
    <div className="flex flex-col w-full max-w-4xl mx-auto py-24 mx-auto stretch">
      {messages.length === 0 && (
        <h1 className="text-6xl font-semibold leading-tight mt-4 mb-16">
          ...
          <br />
          <span className="text-gray-400">Ask me anything (Azure OpenAI)</span>
        </h1>
      )}
